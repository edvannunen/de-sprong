...
// Builds the link that opens iReal Pro's search window pre-filled with a piece's
// title (irealb://search?<title>, per irealpro.com/developer-docs). Confirmed
// on-device (Aug 2026): this only opens the search results — iReal Pro doesn't
// offer a documented way to jump straight to an existing library song by title,
// so the user still has to tap the matching result themselves. That "opens
// directly if already in the library" behaviour belongs to a different scheme
// (the full irealbook://Title=Composer=Style=Key=Chords chart-import link), not
// this one.
export function irealProSearchUrl(title: string): string {
	return `irealb://search?${encodeURIComponent(title)}`;
}
